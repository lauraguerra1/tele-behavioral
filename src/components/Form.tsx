  ...
  const formInputs = Object.keys(formData).map(field => {
    return (
      <div key={field} className={`${field === 'message' || field === 'subject' ? 'col-span-2 flex-col' : ''} flex border-b-1 border-white`}>
        ...
        {field !== 'message'
          ? <input className='focus:outline-none bg-transparent' type={field === 'email' ? 'email' : 'text'} id={field} name={field} onChange={(e) => changeInput(e.target.name, e.target.value)} required={field !== 'subject'} />
          : <textarea  className='focus:outline-none bg-transparent h-32 resize-none' id={field} name={field} onChange={(e) => changeInput(e.target.name, e.target.value)} required/>}
      </div>
    )
  })

  return (
    <form onFocus={resetFeedback} onSubmit={submitForm} className='grid grid-cols-2 gap-4 text-white mt-10'>
      {formInputs}
